...
\begin{figure}[t]
  \centering
    \includegraphics[trim={0 11cm 9cm 0},clip,width=0.9\linewidth]{chapters/background/figures/local_attestation.pdf}
...
\begin{enumerate}
  ...
  \item [\three] After receiving the \texttt{REPORT} from Enclave A, Enclave B executes the following steps:
   ...
   \begin{enumerate}
     ...
     \item Enclave B can then reciprocate by creating a \texttt{REPORT} for enclave A by using the \texttt{MRENCLAVE} value from the REPORT it just received.
