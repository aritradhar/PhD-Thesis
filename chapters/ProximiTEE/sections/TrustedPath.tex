...
%\blue{On top of this attack, relay attack opens up a new avenue of attack. A detailed description of the relay attack that completely breaks the security properties of a trusted path is described in Appendix~\ref{sec:trustedPathAppendix:relay}. An illustration of the same is depicted in Figure~\ref{fig:trustedPathRelay}.}
...
\begin{figure}[t]
 \centering
 \includegraphics[trim={0 12cm 20cm 0},clip,width=0.65\linewidth]{TrustedPath_revised2.pdf}
...
\myparagraph{Local trusted path.} Now, we describe the process of establishing a trusted path to an enclave on a \emph{local} platform. As shown in Figure~\ref{fig:secureInput}, the I/O devices are connected to the \device that is attached to a local computing platform. The trusted path creation proceeds as follows:
...
\begin{figure}[t]
 \centering
  \includegraphics[trim={0cm 8.6cm 18cm 0},clip,width=0.85\linewidth]{SystemDesignRemote_3.pdf}
...
\begin{figure}[t]
  \centering
    \includegraphics[trim={0 7.5cm 9cm 0}, clip, width=0.6\linewidth]{Setup1.pdf}
...
%the application-specific enclave to aid the user understand the identity of the enclave she is communicating with.
