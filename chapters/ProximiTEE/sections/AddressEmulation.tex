...
\subsubsection{The emulation attack} 
...
\subsubsection{Emulation attack implications} 
...
\begin{figure}[t]
 \centering
    \includegraphics[trim={0 7cm 19cm 0},clip,width=0.75\linewidth]{chapters/ProximiTEE/figures/boot_init.pdf}
 \caption[\name noot-time initialization]{\textbf{\name noot-time initialization.} The \device uses a minimal kernel Linux image to boot and load \nameclave on the target platform and seal a platform specific secret to the \device memory.}
 \label{fig:boot-init}
\end{figure}
...
\subsubsection{Boot-time initialization} The boot-time initialization process is performed only once.
...
\subsubsection{Attestation process.} After initialization the target platform runs a regular OS. The attestation process is depicted in Figure~\ref{fig:boot-attest} and proceeds as follows:
...
\subsubsection{Following communication.} 
...
\subsubsection{Comparison to TOFU.} Our second attestation mechanism is a novel variant of the well-known ``trust on first use'' principle. In this section we briefly explain the main benefits of our solution over common TOFU variants. 
