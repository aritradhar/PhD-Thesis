...
%In our test setup we did not perform advanced latency optimizations such as tweaking the OS kernel network stack queues. We acknowledge that such optimizations may reduce the network latency of the adversary and therefore we also analyze the security of our solution against faster adversaries.
...
%. The flood mode effectively fills all the network queues, hence providing us with an intuition of the latencies experienced with a 
%that simulates the scenario where the kernel network stack is optimized for minimum latency. In the following we build on the results presented in Figure~\ref{graph:reroutingDetectionHist}.
